program oxmodel
   ...
   real o(maxlatanz,maxtimeanz,maxzanz)      !concentration of O in molecules per cm^3
   real o3(maxlatanz,maxtimeanz,maxzanz)     !concentration of O3 in molecules per cm^3
   real ox(maxlatanz,maxtimeanz,maxzanz)     !concentration of Ox in molecules per cm^3
   real topo3(maxtimeanz,maxzanz)  !overhead ozone in DU
   real topo3_lat(maxlatanz,maxtimeanz,maxzanz)
   ...
   real albedo !albedo at the location of the model run
      ...
      do ti=2, int(integrationtime)
         ...
         do zi=1, zanz
            ...
            JO2 = make_JO2(jgridz, jgridzanz, jgridsza, jgridszaanz, jgridtopo3, &
               ...
               z(zi), sza(ti), topo3(ti-1, zi), albedo)
            JO3 = make_JO3(jgridz, jgridzanz, jgridsza, jgridszaanz, jgridtopo3, &
               jgridtopo3anz, jgridalbedo, jgridalbedoanz, JO3log_array, &
               z(zi), sza(ti), topo3(ti-1, zi), albedo)

            !JO2 = make_JO2(jgridz,jgridzanz,jgridsza,jgridszaanz,jgridtopo3,jgridtopo3anz,jgridalbedo,jgridalbedoanz,JO2log_array,z(zi),sza(ti),topo3(ti-1,zi),albedo)
            !JO3 = make_JO3(jgridz,jgridzanz,jgridsza,jgridszaanz,jgridtopo3,jgridtopo3anz,jgridalbedo,jgridalbedoanz,JO3log_array,z(zi),sza(ti),topo3(ti-1,zi),albedo)
            ! Calculate new Ox conc. at level zi
            ox(li,ti,zi) = ox(li,ti-1,zi) + (2*JO2*o2(zi) - 2*KO_O3*o(li,ti-1,zi)*o3(li,ti-1,zi)) * timestep*60*60
   
            ! At altitude zi calculate O, O3 from Ox
            o3(li,ti,zi) = KO_O2 * ox(li,ti,zi) * o2(zi)/(JO3 + KO_O2 * o2(zi))
            o(li,ti,zi) = JO3*ox(li,ti,zi)/(KO_O2*o2(zi) + JO3)
         enddo ! finish level loop
         ! Calculate profile of overhead ozone
         call make_topo3(z,zanz,maxzanz,ti,maxtimeanz,o3(li,:,:),topo3)
      enddo ! finish time loop
      topo3_lat(li,:,:)=topo3(:,:)
      write(*,*) topo3_lat(li,:,1)
